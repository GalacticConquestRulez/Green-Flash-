...
DEFAULT_FILM_TITLE = 'Watch the <em>film</em>'
...
def film_band(name, eyebrow, line, title=None, credit='', loop=False, cls=''):
  ...
  <div class="section-head rv"><div class="eyebrow">{eyebrow}</div>
  <h2>{title or DEFAULT_FILM_TITLE}</h2>
  <p class="lead">{line}</p></div>
...
<section class="alt stats-band">{glow()}<div class="wrap">
  ...
  <p class="lead">Added up wall by wall, the walls we have the feet for come to {SQ_FT:,} square feet of painted surface in {spell(len(CITIES))} cities — with more walls in Mexico, Brazil and beyond. The tallest of them stands {TALLEST['dim_h']} feet in {TALLEST['city']}; the widest runs {WIDEST['dim_w']} feet.</p></div>
  <ul class="stats">
...
<section><div class="wrap">
  ...
  <p class="lead">A first look here, the rest on the Work page. The figure over each photograph is how much wall it took — where he has given us the feet.</p></div>
...
def film_hero(p):
    ...
    """
    alt = f"{p['title']} mural by Open Air Gallery in {p['city']}, {p['state']}"
    return page_hero(f'{p["city"]}, {p["state"]}',
...
def progress_band(p):
  ...
  <div class="progress-grid">
    ...
    <div class="progress-words rv rv-d1">
      ...
      <p>Ephraim&rsquo;s own reel from the job: the artwork it was painted
...
def project_page(p):
  ...
  {swipe(slides, f'Photographs of the {p["title"]} mural')}
    ...
    if film:
        ...
        head = f'''<div class="eyebrow">{place}</div>
  <h1>{p['title']}</h1>
